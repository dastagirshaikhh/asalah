...
export default function HeroSection() {
  ...
  const stats = [
    { icon: <Users className="w-6 h-6" />, label: "Students to Train", value: "1 Lakh+" },
    { icon: <Award className="w-6 h-6" />, label: "Faith-Driven", value: "100%" },
    { icon: <BookOpen className="w-6 h-6" />, label: "Coaching Sessions", value: "500+" },
          ...
          >
            <h1 className="text-5xl md:text-6xl font-bold mb-6 tracking-tight relative">
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-green-700 to-green-500">
                Inspired by faith, build unique!
              </span>
              <motion.span
                className="absolute -inset-1 bg-green-500 rounded-full blur-3xl"
                initial={{ opacity: 0 }}
            ...
            <div className="relative inline-block">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="relative z-10">
                <Button
                  size="lg"
                  className="bg-primary text-white hover:bg-green-600 text-lg px-8 py-6 rounded-full transition-colors relative overflow-hidden group"
            ...
            {stats.map((stat, index) => (
              ...
              >
                <motion.div
                  ...
                  className="bg-white rounded-xl p-6 shadow-lg border border-green-100 transition-colors hover:border-green-200"
                >
                  <div className="mb-2 text-primary flex justify-center">{stat.icon}</div>
                  <motion.div
